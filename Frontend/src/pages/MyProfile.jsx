...
export default function MyProfile() {
  ...
  useEffect(() => {
    ...
    const getUser = async () => {
      ...
      setDescripcionProfesional(resolvedUser.descripcion_profesional ?? "");
  ...
  const handleUpdate = async () => {
    ...
    const { data: updatedUser, error: updateError } = await supabase
      ...
      .update({
        ...
        descripcion_profesional: trimmedDescripcion,
      })
      .eq("id", user.id)
      .select()
      .single();
        ...
        <div className="grid gap-3 px-3 py-4 sm:gap-4 sm:px-5 sm:py-6 md:gap-5 md:px-8 md:py-8 lg:grid-cols-[360px_minmax(0,1fr)] lg:px-10 lg:py-10 xl:grid-cols-[430px_minmax(0,1fr)]">
          ...
          <section className="space-y-4 sm:space-y-5 md:space-y-6">
            ...
            <div className="grid gap-3 sm:gap-4 md:gap-5">
              ...
              <div className="space-y-1.5 sm:space-y-2 md:space-y-2.5">
                <label className="text-[10px] font-semibold uppercase tracking-widest text-zinc-500 sm:text-[11px]">Descripcion profesional</label>
                <div className="rounded-xl border border-black/10 bg-white px-3 py-2.5 shadow-[0_6px_16px_rgba(0,0,0,0.03)] sm:rounded-2xl sm:px-4 sm:py-3.5 md:rounded-3xl md:px-5 md:py-4">
                  <textarea
                    value={descripcionProfesional}
                    onChange={(event) => setDescripcionProfesional(event.target.value)}
                    rows={6}
                    className="w-full resize-none bg-transparent text-xs leading-6 outline-none sm:text-sm md:text-base"
                    placeholder="Escribe un resumen de tu perfil profesional, experiencia, habilidades o enfoque laboral..."
                  />
                </div>
              </div>
            </div>
